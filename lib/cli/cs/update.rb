namespace :cs do
  namespace :update do
    ...
    task :generate, [:csv, :output] do |_t, args|
      csv             = args[:csv]
      generated       = []
    ...
    task :process, [:csv, :throttle] do |_t, args|
      csv           = args[:csv]
    ...
    task :template, [:csv, :template] do |t, args|
      ...
      raise 'HELL' unless File.file?(csv) && File.file?(template_file)
